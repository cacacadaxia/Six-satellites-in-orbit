...
for staInxFlat = 1:NumStall
    [satPosECEF(:,:,staInxFlat), vECEF(:,:,staInxFlat) ] = pltState( tEnd, Om0+2*pi/18*(staInxFlat-1) ,toe , om+0.1*(staInxFlat-1) );
    % satPosECEF 3*tEnd*18
end
...
time = 0:tspan:tEnd;
tAcess = [];
for inxTim = 1:length(time)
    t = time(inxTim);
    rStallWgs = satPosECEF(:,inxTim,:);     % 18颗卫星的位置
    vStallWgs = vECEF(:,inxTim,:);          % 18颗卫星的速度
    for stallinx = 1:NumStall
        [ BdegSta(stallinx) , LdegSta(stallinx) ] = XYZsettoBLH(rStallWgs(:,1,stallinx));
    end
...
for i = 1:length(satPosECEF(1,1,:))
    plot3(satPosECEF(1,:,i),satPosECEF(2,:,i),satPosECEF(3,:,i),'-','LineWidth',1.3);hold on;
end
